...
In DSEM syntax, we can first fit a saturated (full-covariance) model using the argument `covs`.  However, we do not do so here to avoid bloat in the vignettes:

```{r, eval=FALSE, echo=TRUE, message=FALSE, fig.width=7, fig.height=7, warning=FALSE}
# Add factors to data
...
Next, we can specify two factors factors while eliminating additional process error and estimating measurement errors.  This requires us to switch to `gmrf_parameterization = "project"`, so that we can fit a rank-deficient Gaussian Markov random field:
