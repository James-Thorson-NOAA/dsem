...
In DSEM syntax, we can first fit a saturated (full-covariance) model, while setting low starting values for off-diagonal covariances (for numerical stability):

```{r, eval=TRUE, echo=TRUE, message=FALSE, fig.width=7, fig.height=7, warning=FALSE}
# Add factors to data
...
Next, we can specify two factors factors while eliminating additional process error and estimating measurement errors.  This requires fitting a rank-deficient Gaussian Markov random field, which we do using the default `gmrf_parameterization = "gmrf_project"`, which fits a full-rank GMRF to variables with non-zero exogenous variance (i.e., factors), and then projects determinstically to the variables with zero exogenous variance (i.e., species-specific indices):
