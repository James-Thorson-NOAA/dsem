...
# library(rmarkdown)
# render( "vignette.Rmd", pdf_document()) #  to build PDF, and perhaps tinytex::latexmk() to install dependencies
```
...
sem = "
  ...
  wolves <-> wolves, 0, SDW
"
fit = dsem( sem=sem, tsdata=data, estimate_delta0=TRUE, upper=0.99, lower=-0.99, quiet=TRUE )
...
## Compile
library(TMB) # needed to do summary for m2
...
ggarrange( p1 + scale_x_continuous(expand = c(0.4, 0.1)),
           p2 + scale_x_continuous(expand = c(0.4, 0.1)),
