...
lake_washington$Temp = lake_washington$Temp - 10

# Format
Z = ts(cbind(
  lake_washington[,c("Temp","Daphnia","Leptodora","Cryptomonas")],    
...
We then fit this model

```{r fit_varying, echo=TRUE, message=FALSE}
fit = dsem(
  tsdata = Z,
  sem = sem,
  estimate_mu = c("Daphnia","Leptodora","Cryptomonas","alpha","beta"),  # "TP", "Year",
  control = dsem_control(
    trace = 1,
    use_REML = TRUE,  # Easier to get intercepts from sdrep$cov.fixed, but also slower
    eval.max = 10000,
    iter.max = 10000,
    getsd = TRUE,
    getJointPrecision = TRUE, # Alternative to get SEs
    newton_loops = 0,
    quiet = TRUE
  )
)
...
plot_interval = function( fit, var, name1, name2, n = 1000, prob = c(0.025,0.975), f_y = identity, ... ){
  ...
  beta = rmvnorm_prec( prec = fit$sdrep$jointPrecision, n = n )[index,]
...
ggraph(layout) +
  geom_edge_link2(
    ...
    aes( label = label, linetype = type, col = ifelse(is.na(p)|p<0.05,"black","grey"), filter = straight ),  # , edge_width = ifelse(type == "dotted", 0.8, 0.8)
    vjust = -0.2,
    hjust = 0.4,
  ) +
  geom_edge_arc(
    ...
    start_cap = ggraph::circle(0, 'mm'),
    strength = -1,
    aes( label = label, linetype = type, col = col, filter = !straight ),  # , edge_width = ifelse(type == "dotted", 0.8, 0.8)
    vjust = -0.2,
    hjust = 0.4,
  ) +
  geom_node_point(
